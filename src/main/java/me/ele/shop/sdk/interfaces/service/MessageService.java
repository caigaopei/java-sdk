...
public interface MessageService {
     ...
     */
    List<OMessage> getNonReachedOMessages(@NotNull(message = "应用ID不能为空") Integer appId) throws ServiceException;
     ...
     * @desc 推送成功的消息系统会给以标记成功，该接口用于获取未标记成功的消息，每次返回最多10条。返回的消息在接口调用后会标记为已经推送。
     */
    List<String> getNonReachedMessages(@NotNull(message = "应用ID不能为空") Integer appId) throws ServiceException;
}
